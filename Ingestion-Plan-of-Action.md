...
**Decision (2026-09-21) — go-forward sourcing per state:**
- **NC:** build a scraper against the `nccemetery.org` HTML table (name + city).
- **FL, TX:** proceed with the Ch. 119 / supervised-entity-list public-records requests described above.
- **SC:** buy the LLR licensee rosters (~$10/license type).
- **PA:** skip the scraping/RTK path — use the Google Places API (Text Search) to look up each cemetery/funeral home by name + city and pull phone/website directly, same pattern as the HOA management-company enrichment below (§6.2).
...
2. **TREC certificate PDFs — are they machine-parseable** (text-layer PDFs) or scanned images requiring OCR? **Now the #1 open item, and promoted from "swings the estimate" to "gates the vertical."** The 2026-08-17 extract confirmed the CSV carries no street address and no contact of any kind (§6.1.1), so the PDFs are the only path to a usable Texas HOA list. Certificate URLs are in hand for all 17,071 associations. **Action: download ~20 certificates spanning Harris, Dallas, Travis, and Bexar** — county recording practice varies, so a single-county sample will mislead — **and check for a text layer.** Text layer means `pdfplumber` plus per-county regexes, a few days. Scanned means OCR, worse accuracy, and a review queue.
   - **Partial result, 2026-09-21 (n=5, one per Harris×2/Dallas/Travis/Bexar via live `data.texas.gov` pull):** 4 of 5 sampled certificates had **no text layer** (`pdftotext` returned empty; producers were `GdPicture Managed PDF Plugin` and `Epson Scan 2` — both scan tools) — Harris (both samples), Travis, and Bexar. Only the Dallas sample had a clean extractable text layer.
   - **Second sample, 2026-09-21 (n=3, Trinity Estates / Townhomes at Liberty Branch / Vacek Point, distinct associations):** all three had **no text layer** and, notably, an identical technical signature — `Producer: OpenPDF 2.0.5`, every page a single 1-bit CCITT Group 4 fax-encoded image at 2550×3300px / 300ppi (standard scanned-letter-page dimensions), zero embedded fonts (`pdffonts` empty), 2–3 pages each. That uniformity across three different associations suggests a common recording/e-filing pipeline producing this exact container format, not per-county scanner variance.
   - **Combined read (n=8 total):** 7 of 8 sampled certificates are scanned images with no text layer; the CCITT/OpenPDF signature looks like the dominant format rather than an edge case. **This is now firm enough to commit to OCR as the primary path** (Tesseract or Document AI — not yet installed locally, `tesseract` is not on PATH) rather than treating `pdfplumber` text-layer extraction as the default and OCR as a fallback. Still worth finishing the ~20-certificate sweep across more counties before finalizing the parser-registry design, but plan resourcing around OCR + a review queue, not around per-county regexes on extracted text.
   - **Confirmed target fields, 2026-09-21 (rendered + visually read the Trinity Estates / Tarrant County cert, page 1):** the form is a clean typed template, numbered fields 1–8, not handwritten (aside from the signature). Fields 5 and 6 are the ones that matter and are labeled verbatim on the form itself: **"5. Name and mailing address of the Association"** and **"6. Name, mailing address, phone number & email for designated representative"** — the latter is the exact managing-agent/phone/email target described above. Example values from this sample: Association mailing address `Trinity Estates POA, Inc., c/o Goodwin & Company, PO Box 203310, Austin, TX 78720`; designated representative `Goodwin & Company, PO Box 203310, Austin, TX, 855.289.6007, info@goodwin-co.com`. Field 7 (website) is also present and a cheap bonus grab (`https://dtrin.sites.townsq.io/`, `https://goodwin-co.com/`).
   - **OCR outlook, revised upward:** because the statute (§209.004) mandates specific numbered content but not a specific layout, filings vary in visual template — but where a template is used consistently by a filer/management company, the numbered field labels are literal text on the page, so post-OCR extraction can anchor on "6." → next numbered field rather than needing free-form parsing. Expect Tesseract accuracy to be good on these (clean 300dpi typed-text scans are close to ideal OCR input); the real remaining risk is **template diversity across counties/preparers**, not OCR quality on any single template. **Decision (2026-09-21):** in parallel with resolving this, add a Google Places (Text Search) lookup on association name + city + ZIP as a cheap supplemental enrichment layer for phone/website — it won't replace the PDF for mailing address/managing-agent legal fields, but for professionally-managed associations it can fill phone/website without waiting on OCR. Self-managed/volunteer-run associations likely won't have a Maps listing, so this is a supplement, not a substitute. This is the same Google Places lookup used for PA deathcare (§6.3).

**Build source (2026-09-21):** two prior-prototype repos were reviewed for reusable code — `juniperlandscaping/juniper-crm-HOA-pipeline` and the private shared library it depends on, `juniperlandscaping/juniper-crm-shared`. Verdict: cherry-pick two modules and adapt them into `connectors/lib/`, skip everything else in both repos.
- **Port:** `juniper-crm-shared/scrapers/google_maps.py` (`GoogleMapsScraper` — Places API "Text Search" with retry/backoff and an address-based fuzzy-match scorer to disambiguate results) and the `_extract_emails_from_website()` + robots.txt-respecting rate limiter in `juniper-crm-shared/scrapers/base.py`. Both are self-contained and reasonably well-written; adapt their output to write into `staging.resolved_contact` instead of the source repos' flat `management_companies`/`hoa_properties` tables.
- **Do not port:** `scrapers/hoa_usa.py` / `ArcGisScraper` (a different HOA-discovery strategy — county ArcGIS parcel scraping — superseded by the TREC-CSV approach this plan already commits to; `connectors/lib/arcgis.py` already covers FeatureServer paging against our schema); `scrapers/sunbiz.py` (calls a **paid** third-party API, `sunbizdaily.com`, where this plan calls for the **free** Sunbiz SFTP bulk file — wrong source); `scrapers/dbpr.py`, `fl_county_acreage.py`, `management_company_dedup.py` (all write into the other repos' flat MySQL schema, not our account/location/contact model — not reusable without a full rewrite).
